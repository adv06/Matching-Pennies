...
for i in range(training_steps):
    ...
    with torch.no_grad():
        ...
        history_A.append(pA)
        history_B.append(pB)
        exploit_A = max(2*pB - 1, 1 - 2*pB) - (pA * (2*pB - 1) + (1-pA) * (1 - 2*pB))
        ...
        gen_A_val = mlp_A(input)
        gen_B_val = mlp_B(input)
        history_lyuponov_A.append((0.5 * (torch.log(torch.tensor(0.5, dtype=torch.float32)) - torch.log(gen_A_val))).sum().item())
        history_lyuponov_B.append((0.5 * (torch.log(torch.tensor(0.5, dtype=torch.float32)) - torch.log(gen_B_val))).sum().item())
...
ax1.set_title("Policy Convergence (Vanilla)")
ax1.legend()
...
plt.tight_layout()
plt.savefig("convergence_vanilla.png")
plt.show()